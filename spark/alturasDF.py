...
df = spark.read.csv("hdfs://namenode:9000/user/root/input/alturasRev.csv", header=False, inferSchema=True)

# Rename columns to 'sexo' and 'altura'
df = df.withColumnRenamed("_c0", "sexo").withColumnRenamed("_c1", "altura")

# Convert 'altura' to float and correct heights that are in meters
df = df.withColumn("altura", when(col("altura") < 10, col("altura") * 100).otherwise(col("altura")))
